...
class ProductPage(BasePage):
    ...
    def should_be_right_book_name(self, bookToCompare):
        assert self.browser.find_element(*ProductPageLocators.BOOK_NAME).text == bookToCompare, "V korzine net knig."

    def should_be_right_price_for_book(self, priceToCompare):
        assert self.browser.find_element(*ProductPageLocators.PRICE_NUMBER).text == priceToCompare, "Ne pravilno ukazana cena za knigu."

    def should_not_be_success_message(self):
        assert self.is_not_element_present(*ProductPageLocators.SUCCESS_MESSAGE), "Soobshenie 'success' NE doljno bit otobrajeno!"

    def should_disappear(self):
        assert self.is_disappeared(*ProductPageLocators.SUCCESS_MESSAGE), "Soobshenie NE otobrajeno!"
